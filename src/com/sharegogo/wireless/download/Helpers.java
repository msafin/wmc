...
public class Helpers {
    ...
    // Add this to support CU customization
    public static boolean mContinueDownload;
    public static String mNotificationClass;
    ...
    public static String generateSaveFile(
            ...
            String notificationClass) throws GenerateSaveFileError {
    ...
    private static File locateDestinationDirectory(Context context, String mimeType,
        ...
        return getExternalDestination(context, contentLength, mimeType);
    }

    private static File getExternalDestination(Context context, long contentLength, String mimeType) throws GenerateSaveFileError {
        if (!isExternalMediaMounted()) {
            throw new GenerateSaveFileError(Downloads.Impl.STATUS_DEVICE_NOT_FOUND_ERROR,
                    "external media not mounted");
        }
        ...
        String dir = null;
        Cursor cur = null;
        String[] projection = new String[] {"path"};
        Uri allCategory = Uri.parse(Constants.FILE_MANAGER_URI);
        ...
        if (mimeType.startsWith("audio/")) {
        	...
        	selectionStr = "download";
        }

        try {
            cur = context.getContentResolver().query(allCategory, projection,
            		selectionStr, null, null);
            if (cur!=null && cur.moveToFirst()) {
                dir = cur.getString(cur.getColumnIndex("path"));
                if (dir == null) {
                    dir = root.getPath() + Constants.DEFAULT_DL_SUBDIR;
                }
            } else {
            	dir = root.getPath() + Constants.DEFAULT_DL_SUBDIR;
            }
            base = new File(dir);
            Log.v(Constants.TAG, "Helper.getExternalDestination():download directory is:" + dir);
        } catch (IllegalStateException e) {
            Log.e(Constants.TAG, "Helper.getExternalDestination():Query encounter exception");
        ...
        // base.mkdirs() will make complete directory
    ...
    private static String chooseUniqueFilename(int destination, String filename,
        ...
        if(true) {
            // If not support CU customization, follow google default flow
